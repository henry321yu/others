...
void setup() {
  ...
  Serial.printf("Watchdog timeout : %d s\n",wdtt/1000);
...
void loop() {
  ...
  nowmillis = millis();
  if (nowmillis - last_sample_time >= SAMPLE_INTERVAL_MS) {    
    last_sample_time = nowmillis;
    i++;
    float freq = nowmillis / 1000;
    freq = i / freq;

    acc_data();
    float magnitude = sqrt((ax - int_ax) * (ax - int_ax) + (ay - int_ay) * (ay - int_ay) + (az - int_az) * (az - int_az));
    String acc_String_dataString = (ax, 5) + "," + String(ay, 5) + "," + String(az, 5);
    String data = timeStamp() + "," + acc_String_dataString + "," + String(magnitude, 5) + "," + String(events) + "," + statuss + "," + String(freq, 2) + "," + nowfile;
    ...
    if (magnitude > THRESHOLD_G) {
      if (!triggered) {
        ...
        triggerEndTime = nowmillis + 2UL * 60 * 1000 * SETT;  // ➜ 後 2 分鐘
...
String timeStamp() {
  static uint32_t lastMillis = nowmillis;
  static time_t lastSecond = now();

  uint32_t nowMillis = nowmillis;
