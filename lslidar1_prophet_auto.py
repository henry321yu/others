...
def process_and_plot():
    ...
    T_all = T_all.dropna(subset=['time_a'])
    ...
    latest_time = T_all['time_a'].iloc[-1]
    oldest_time = T_all['time_a'].iloc[0]
    latest_s = T_all['program_time'].max()
    T_recent = T_all[T_all['time_a'] >= latest_time - timedelta(hours=model_hr)].copy()
    ...
    plt.plot(T_all['time_a'], data, '.', markersize=1.2, label=f'全數{latest_s:.2f}秒資料')
