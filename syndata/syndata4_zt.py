...
def receiver_once():
    ...
    for file in os.listdir(FOLDER):
        if file.endswith(".tmp"):
            ...
            try:
                os.remove(tmp_path)
                print(f"[CLEANUP] 刪除暫存檔: {tmp_path}")
            except Exception as e:
            ...
            try:
                ...
                with open(tmp_file_path, 'wb') as f:
                    ...
                    while received < filesize:
                        ...
                        eta_str = time.strftime('%M:%S', time.gmtime(eta))
                        print_name = (filename[:disnamelen-3] + '...') if len(filename) > disnamelen else filename
                        print(f"\r[RECEIVE] ↓ {print_name}: {received/filesize*100:.2f}% ({speed:.2f} MB/s, ETA: {eta_str})", end='', flush=True)

                if os.path.exists(final_file_path):
                    os.remove(final_file_path)
                os.rename(tmp_file_path, final_file_path)
                print('\r' + ' ' * (disnamelen + extranamelen) + '\r', end='')
                print(f"[RECEIVE DONE][From:{addr[0]}] {filename} ({filesize / megabyte:.2f} MB)")
                receiving = 0
            except Exception as e:
                receiving = 0
                ...
                if tmp_file_path and os.path.exists(tmp_file_path):
                    try:
                        ...
                        print(f"[CLEANUP] 刪除不完整檔案 {tmp_file_path}")
                    except Exception as cleanup_error:
                        print(f"[CLEANUP ERROR] {cleanup_error}")


def main_loop():
    print(f"[INFO][{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 開始輪流同步模式")
