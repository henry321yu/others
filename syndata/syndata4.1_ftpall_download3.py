...
def reconnect_ftp(ftp, host, user, password):
    try:
        ftp.quit()
    except:
        pass

    print("[RECONNECT] FTP reconnecting...")
    ftp = ftplib.FTP()
...
def scan_and_download(ftp, remote_folder, local_folder):
    ...
    ftp_download_all(ftp, remote_folder, local_folder)
...
def main_loop(ftp, ftp_folder, download_folder):
    ...
    while True:
        try:
            if not ftp_is_alive(ftp):
                ftp = reconnect_ftp(ftp, ftp_host, ftp_user, ftp_pass)
            ...
            try:
                ...
                print(f"[RECONNECT FAILED] {e2}，5 秒後再試")
                ftp = None
                time.sleep(5)
                continue
...
if __name__ == "__main__":
    ...
    while ftp is None:
        try:
            ...
            print(f"[CONNECT ERROR] {e}，5 秒後重試")
            ftp = None
            time.sleep(5)
