...
# 保留最近 500 筆資料
elapsed_list = deque(maxlen=500)
board2_temp_list = deque(maxlen=500)
tem_list = deque(maxlen=500)
frequency_list = deque(maxlen=500)
dfrequency_list = deque(maxlen=500)
        ...
        if len(data_list) >= i_max_points:
            ...
            for vert_angle, azimuth, distance, intensity in data_list:
                ...
                if elapsed_s_total > printclock:
                    ...
                    elapsed_list.append(elapsed_s_total)
                    board2_temp_list.append(board2_temp)
                    tem_list.append(tem)
                    frequency_list.append(frequency)
                    dfrequency_list.append(dfrequency)
                    plotter_thread()
                    print(f"{current_time}, {elapsed_s_total:.2f}s, {board2_temp:.2f}°C, {tem}°C, {frequency:.2f} kHz, {dfrequency:.2f} Hz")
...
def plotter_thread():        
    t=max(elapsed_list)
    plt.clt()             # 清螢幕
    plt.plotsize(200, 30) # 調整尺寸        

    plt.plot(elapsed_list, board2_temp_list, marker='dot', color='green')
    plt.plot(elapsed_list, tem_list, marker='dot', color='blue')
    plt.plot(elapsed_list, dfrequency_list, marker='dot', color='yellow')
    plt.xlim(left=max(0, t-10))
    plt.ylim(0, 60)
