...
def trigger_scan_pywinauto():
    ...
    try:
        ...
        # # 2. (可選，但推薦) 讓視窗準備就緒
        # logging.info("讓視窗準備就緒中 ...")   # 12/3 註解掉 改成 scan_button.invoke()
        # main_window.set_focus()               # 12/3 註解掉 改成 scan_button.invoke()
        # main_window.wait('ready', timeout=10) # 12/3 註解掉 改成 scan_button.invoke()
        ...
        if scan_button.exists() and scan_button.is_enabled():
            ...
            # scan_button.click_input() # 12/3 註解掉 改成 scan_button.invoke()
            scan_button.invoke()
