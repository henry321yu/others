...
class ArcSARMonitor:
    ...
    def trigger_alarm(self, top_3_details):
        ...
        if not DISCORD_WEBHOOK_URL:
            print("[Alarm] 未設定 Discord Webhook，略過推播")
            return
        ...
        try:
            ...
            if resp.status_code == 204:
                print("[Alarm] Discord 告警已成功送出")
            else:
                print(f"[Alarm] Discord 回傳異常: {resp.status_code}")
        except Exception as e:
            print(f"[Alarm] Discord 推播失敗: {e}")
    ...
    def process_rebound(self, diff_dphase_path):
        ...
        # 組通知內容（不論是否超標）
        status_str = "ALARM" if is_alarm else "NORMAL"

        notify_msg = (
            f"Status: {status_str}\n"
            f"File: {os.path.basename(diff_dphase_path)}\n"
            f"Max: {max_rebound:.3f} mm\n"
            f"Avg: {avg_rebound:.3f} mm\n"
        )
        top_3_str = "None"
        if is_alarm:
            notify_msg += f"\nTop3:\n{top_3_str}"
        ...
        self.trigger_alarm(notify_msg) # ⭐ 每次都有掃描結果就通知
        return is_alarm
