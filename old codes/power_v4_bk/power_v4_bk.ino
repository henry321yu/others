...
int circletime = 600; // verson.1 was 150 + 50
int ondelay=1,offdelay=4;
...
int relayA[] = {6, 5, 4}; // set relay pin,power pin,M pin
int relayB[] = {9, 8, 7}; // set relay pin,power pin,M pin
int r[] = {0, 1, 2, 0, 1, 2};
byte power;
...
void loop() {
  ...
  if (a1on == 0 && a1off == 0) { //initialization
    a1on = ondelay; a1off = offdelay;
  }
  if (currentmillis >= a1off) { //ssrA off
    a1off = a1off + chargecircletime; digitalWrite(relayA[r[0]], LOW);
    Serial.print(currentmillis);Serial.print(" ");
    Serial.println(F(" a1off "));
  }
  if (currentmillis >= a1on) { //ssrA on
    a1on = a1on + chargecircletime; digitalWrite(relayA[r[0]], HIGH);
    Serial.print(currentmillis);Serial.print(" ");
    Serial.println(F(" a1on "));
  }


  if (currentmillis >= a2off) { //powerA off
    a2off = a2off + circletime; 
    a1off = a1off + ontime;
    a1on = a1on + ontime;
    b1off = b1off + ontime; //added
    b1on = b1on + ontime;
    ...
    Serial.print(currentmillis);Serial.print(" ");
    Serial.println(F(" powerAoff "));
    Serial.println(F(" a2off "));
    Serial.println(F(" a1off "));
    Serial.println(F(" b1off "));
    Serial.println(F(" b2off "));
  }
  if (currentmillis >= a3on) { //relayA on
    a3on = a3on + circletime; digitalWrite(relayA[r[2]], HIGH);
    Serial.print(currentmillis);Serial.print(" ");
    Serial.println(F(" a3on "));
  }
  if (currentmillis >= a3off) { //relayA off
    a3off = a3off + circletime; digitalWrite(relayA[r[2]], LOW);
    Serial.print(currentmillis);Serial.print(" ");
    Serial.println(F(" a3off "));
  }
  if (currentmillis >= a2on) { //powerA on
    a2on = a2on + circletime; digitalWrite(relayA[r[1]], HIGH);
    Serial.print(currentmillis);Serial.print(" ");
