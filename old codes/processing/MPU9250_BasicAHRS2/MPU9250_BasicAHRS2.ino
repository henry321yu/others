...
float GyroMeasError = PI * (4.0f / 180.0f);   // gyroscope measurement error in rads/s (start at 4 deg/s)
...
/* COMMENTED : not using MS5637
  ...
  delay(1000);  
*/
/* COMMENTED : not using interrupt pin
...
*/   
    readMPU9250Data(MPU9250Data); // Read accel,temp,gyro 
...
/*  COMMENTED : not using interrupt handler
    if(newMagData == true) {
...
//  MahonyQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f, my, mx, mz);
  ...
  // Serial print and/or display at 10Hz 
  delt_t = millis() - count;
  if (delt_t > 100) { // print data at 10Hz (=100ms)
    ...
    Serial.print(" gz = "); Serial.print( gz, 3); Serial.println(" deg/s");
...
/* COMMENTED : not using MS5637
    MS5637_D1 = MS5637Read(ADC_D1, OSR);  // get raw pressure value
    MS5637_D2 = MS5637Read(ADC_D2, OSR);  // get raw temperature value
    dT = MS5637_D2 - Pcal[5]*pow(2,8);    // calculate temperature difference from reference
...
// Second order corrections
 ...
 //
     ...
     Pressure = (((MS5637_D1*SENS)/pow(2, 21) - OFFSET)/pow(2, 15))/100;  // Pressure in mbar or kPa
