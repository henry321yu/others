...
int freq = 1000; //set frequency (ms)
int circlet = 2 * freq;
...
void setup()
{
  ...
  delay(100);
  Serial.println(F("waiting for power on..."));
  // relay
...
void loop() {
  ...
  if (digitalRead(buttonpin) == LOW) {
    if (currentmillis >= interval1 && power == 0) {
      ...
      interval2 = interval1 - freq;
      Serial.print(currentmillis);
      Serial.print(F(" relay "));
      Serial.print(relay[r[0]]); digitalWrite(relay[r[0]], LOW);
      Serial.print(F(" on "));
      power = 1;
      writeDataToCard();
    }
    if (currentmillis >= interval2 && power == 1) {
      Serial.print(currentmillis);
      Serial.print(F(" relay "));
      Serial.print(relay[r[0]]); digitalWrite(relay[r[0]], HIGH);
      Serial.print(F(" off "));
      power = 0;
      writeDataToCard();
    }
  ...
  else if (currentmillis % 3000 < 10) {
    Serial.println(F("power off.. "));
    interval1 = 0, interval2 = 0; power = 0;
  }

  if ( msgType == MT_NAV_STATUS ) {
    Serial.print(F("gpsFix:"));    Serial.print(ubxMessage.navStatus.gpsFix);
  }
  else if ( msgType == MT_NAV_POSLLH ) {
    Serial.print(F(" lat/lon: "));
    Serial.print(ubxMessage.navPosllh.lat / 10000000.0f, 7);
    Serial.print(F(","));
    Serial.print(ubxMessage.navPosllh.lon / 10000000.0f, 7);
    Serial.print(F(" height: "));
    Serial.print(ubxMessage.navPosllh.hMSL / 1000.0f, 4);
    Serial.print(F(" hAcc: "));    Serial.print(ubxMessage.navPosllh.hAcc / 1000.0f, 4);
    Serial.print(F(" vAcc: "));    Serial.print(ubxMessage.navPosllh.vAcc / 1000.0f, 4);
    Serial.println();
  }
...
void writeDataToCard() {
  ...
  Serial.print((TWtime[1]), 0);
  Serial.print(F(":"));
  Serial.print((TWtime[2]), 0);
  Serial.print(F(":"));
  Serial.print((TWtime[3]), 2);
  Serial.println();
}
