...
log_queue_full = packet_queue_full = 0
...
def file_logger():
    ...
    def get_existing_max_index(date_prefix):
        max_index = -1
        for file_name in os.listdir():
            if file_name.startswith(f"{base_name}_{date_prefix}_") and file_name.endswith(extension):
                try:
                    index = int(file_name[len(base_name)+8:-len(extension)])  # "_YYMMDD_XX.txt"
                    if index > max_index:
                        max_index = index
                except ValueError:
                    continue
    ...
    # 初始索引
    file_index = get_existing_max_index(last_date_str) + 1 if get_existing_max_index(last_date_str) != -1 else 0

    def get_file_path(date_str, index):
        return f"{base_name}_{date_str}_{index:02d}{extension}"

    current_file = open(get_file_path(last_date_str, file_index), "w")
    current_file.write("time,program_time,vert_angle,azimuth,distance,board2_temp,ax,ay,az,temperture,points,frequency(khz),program_frequency\n")
        ...
        try:
            ...
            if line:
                ...
                if len(parts) > 1:
                    ...
                    if current_date_str != last_date_str or program_time - tt > 86400:
                        tt = program_time
                        current_file.close()
                        last_date_str = current_date_str
                        file_index = get_existing_max_index(last_date_str) + 1
                        current_file = open(get_file_path(last_date_str, file_index), "w")
                        current_file.write("time,program_time,vert_angle,azimuth,distance,board2_temp,ax,ay,az,temperture,points,frequency(khz),program_frequency\n")
...
def pointcloud_updater():
    global start_time_ns, i_max_points, frame_count, dframe_count, ax, ay, az, tem, printclock, board2_temp
    while True:
        ...
        if len(data_list) >= i_max_points:            
            ...
            for vert_angle, azimuth, distance, intensity in data_list:
                ...
                if(elapsed_s > printclock):
                    printclock = printclock + 5
                    print(f"{current_time}, {elapsed_s:.3f}s, {vert_angle}, {azimuth:.2f}, {distance:.2f}, {board2_temp:.2f}°C, {ax}, {ay}, {az}, {tem}°C, Points:{len(data_list)}, {frequency:.2f} kHz, {dfrequency:.2f} Hz")
                    log_line = f"{current_time},{elapsed_s:.3f},{vert_angle},{azimuth:.2f},{distance:.2f},{board2_temp:.2f},{ax},{ay},{az},{tem},{len(data_list)},{frequency:.2f},{dfrequency:.2f}"
